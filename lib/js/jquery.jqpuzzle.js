...
function updateSolver(currentShuffleOrder){
	board = currentShuffleOrder; //shuffles[0];
};


function initJQPuzzle() {	
	...
	$.fn.jqPuzzle = function(settings, texts) {
		...
		var defaults = {
			...
			success: {
				...
				callback: 'mbjAddAwardBean',				// callback a user-defined function [function]
			...
			style: {
				...
				backgroundOpacity: 0.25	// opacity of the original image behind the pieces
		...
		// report the linear index from a row/col pair (zero-based)
		var updateLinearPosition = function(row, col) {
			linearPosition = parseInt(row)*cols + parseInt(col);
		...
		return this.filter('img').each(function(){
			...
			$srcImg.one('load', function() {
				...
				var coveredWidth  = cols + (cols-1)*style.gridSize;
				var coveredHeight = rows + (rows-1)*style.gridSize;
				
				// recalc if overlap
				if(overlap) {
					coveredWidth  -= (cols-1);
					coveredHeight -= (rows-1);
				}
				

				console.log("Covered Width: " + coveredWidth);
				console.log("Covered Height: " + coveredHeight);
				// make sure to get the original image size, not scaled values
				// in mozilla, width() and height() do not work with hidden elements
				// original values "auto"

				// (Disabled to enforce consistent 500px width)
				...
				var totalPieceWidth  = "33.33%";
				var totalPieceHeight = "33.33%";
				...
				var getOffset = function(row, col) {
					...
					if(overlap) {
						offset.left -= col * 33.33;
						offset.top  -= row * 33.33;
					}
				...
				var shuffle = function(rounds, speed) {
					...
					for(var i = 0; i < rounds; i++) {
						...
						for(var j = 0; j < shuffles[i].length; j++) {
							...
							/* [sx] // console.log("Piece to be moved: " + $piece.pieceIndex); */
					...
					currentShuffleOrder = shuffles[0];
					updateSolver(currentShuffleOrder);
				...
				$pieces.click(function() {
					...
					linearSource = getLinearPosition(source.row, source.col);
					linearDest = getLinearPosition(dest.row, dest.col);
					...
					currentID = parseInt($piece.attr('id'));
					currentPos = parseInt($piece.attr('current'));
					holePos = parseInt(linearDest);
					console.log("Piece " + currentID + " is in position " + currentPos + ", moving to position " + holePos);

					holeID = currentShuffleOrder[holePos];
					currentShuffleOrder[holePos] = currentID;
					currentShuffleOrder[currentPos] = holeID; 
				...
				pieceHandler(currentID, currentPos, holeID, holePos);
...
//      8tile_solver
//      
//      A solution finder for jqpuzzle.js
//      Built on 8tile.js (https://github.com/smoran02/8-tile-puzzle)
...
function clearHint(thisid) {
       	$(thisid).removeClass('hint');
        console.log("Hint cleared.");
        stopAnswers();
        hintGiven = false;
}
...
function solvePuzzle() {
    ...
    console.log("Attempting to solve current configuration...");
    solveInit = true;
    
    if (xOpen.length != 0) {
        xOpen.splice(0, xOpen.length);
    }
    if (xClosed.length != 0) {
        xClosed.splice(0, xClosed.length);
    }

    var initialState = new Node(board);
    initialState.calcHeuristics();
    xOpen.push(initialState);




    while (xOpen[0].manDistance != 0) {

        for (var i = 0; i < 9; i++) {
            if (xOpen[0].numbers[i] == 0) {																// !!!
                blankid = i;
            }
        }
        ...
        for (var i = 0; i < 9; i++) {
            if ((i - 3 == blankid) || (i + 3 == blankid) ||
                (parseInt(i / 3) == parseInt(blankid / 3) && (Math.abs(i - blankid) == 1))) {
                    var nextMove = xOpen[0].numbers[i];                                                  // !!!
                    var nextBoard = xOpen[0].numbers.slice();
                    var temp = nextBoard[i];
                    nextBoard[i] = nextBoard[blankid];
                    nextBoard[blankid] = temp;
                    var addToQueue = new Node(nextBoard);
                    addToQueue.moves = xOpen[0].moves.slice();
                    addToQueue.moves.push(nextMove);
                    addToQueue.calcHeuristics();
                    if (nextMove != xOpen[0].moves[xOpen[0].moves.length - 1]) {
                        addStateToQueue(addToQueue);
                    }
            }
        }
        xClosed.push(xOpen[0]);
        xOpen.shift();
        xOpen.sort(function(c, d) {
...
function addStateToQueue(addToQueue) {
    var added = false;
    for (var i = 0; i < xOpen.length; i++) {
        if (addToQueue.numbers == xOpen[i].numbers) {
            added = true;
            if (addToQueue.fOfx < xOpen[i].fOfx) {
                xOpen.splice(i, 1);
                xOpen.push(addToQueue);
            }
        }
    }

    for (var i = 0; i < xClosed.length; i++) {
        if (addToQueue.numbers == xClosed[i].numbers) {
            added = true;
            if (addToQueue.fOfx < xClosed[i].fOfx) {
                xClosed.splice(i, 1);
                xOpen.push(addToQueue);
            }
        }
    ...
    function Node(boardArray) {
        ...
        /* TESTING */ this.numbers = boardArray;
        this.fOfx = -1;
        this.moves = [];
        ...
        this.calcHeuristics = function() {
            ...
            for (var i = 0; i < 9; i++) {
                var x = Math.max(i, this.numbers[i]);
                var y = Math.min(i, this.numbers[i]);
                if (this.numbers[i] == 0) {
                    x = y;
                }
                while (parseInt(x / 3) != parseInt(y / 3)) {
    ...
    function makeSwap() {
        ...
        var thisid = -1;
        var x = -1;
        var nextMove = xOpen[0].moves[counter];
        for (var i = 0; i < 9; i++) {
            if (board[i] == nextMove) {
    ...
    function returnId() {
        for (var i = 0; i < 9; i++) {
            if (board[i] == xOpen[0].moves[0]) {
                var attrval = "[current=" + i + "]";
                return $(attrval);
...
function pieceHandler(currentID, currentPos, holeID, holePos){
    console.log("Piece Handler active.");
    hintActive = false;
    ...
    // Remove hint class from clicked tile
    clearHint();


		if (gameWon == false &&
        ((thisid - 3 == blankid) ||                                                                     // If hole is above current piece or...
        (thisid + 3 == blankid) ||                                                                      // If hole is below current piece or...
        ((parseInt(thisid / 3) == parseInt(blankid / 3)) && (Math.abs(thisid - blankid) == 1)))) {      // If hole is to the left or right of piece
            moves++;
            hintGiven = false;
            stopAnswers();
